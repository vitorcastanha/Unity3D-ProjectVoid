...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        [Header ("Small change to suit the project")]
        public int dontClipLayer;           // don't clip against objects with this tag (useful for not clipping against the targeted object)
        ...
        private void LateUpdate()
        {
            ...
            {
                if ((!cols[i].isTrigger) &&
                    !(cols[i].attachedRigidbody != null && cols[i].gameObject.layer == dontClipLayer))
                {
            ...
            for (int i = 0; i < m_Hits.Length; i++)
            {
                ...
                if (m_Hits[i].distance < nearest && (!m_Hits[i].collider.isTrigger) &&
                    !(m_Hits[i].collider.attachedRigidbody != null &&
                        m_Hits[i].collider.gameObject.layer == dontClipLayer))
                {
